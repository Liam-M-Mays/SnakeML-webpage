"""
Memory Augmented Policy Optimization (MAPO) implementation.

MAPO combines Mixture of Experts with PPO-style policy optimization.
Uses multiple expert networks whose outputs are blended based on
...
class GatingNetwork(nn.Module):
    ...
    Uses temperature scaling for sharper/softer expert selection.
    """
    ...
    def __init__(self, in_features: int, num_experts: int, hidden_dim: int = 64):
        ...
        # Deeper gating network for better state understanding
        ...
        # Learnable temperature for softmax (starts at 1.0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        logits = self.net(x)
        # Temperature-scaled softmax (lower temp = sharper selection)
        # Clamp temperature to avoid division by zero
        temp = torch.clamp(self.temperature, min=0.1, max=2.0)
        return F.softmax(logits / temp, dim=-1)


class MAPO(BaseNetwork, nn.Module):
    """
    Memory Augmented Policy Optimization for reinforcement learning.

    Combines Mixture of Experts architecture with PPO-style training.
    ...
    Training uses PPO-style on-policy updates with clipped objective.
    """
    ...
    def __init__(self, input_dim: int, action_dim: int = 3, params: dict = None,
        ...
        self.lr = float(params.get('lr', 0.0003))
        ...
        # Store last blend weights for real-time visualization
        ...
        self.gating = GatingNetwork(input_dim, self.num_experts, hidden_dim=128)
        ...
        self.expert3 = ExpertLinear(self.num_experts, hidden_dim, hidden_dim)
        ...
        print(f"MAPO initialized on device: {self.device} with {self.num_experts} experts")
    ...
    def forward(self, state: torch.Tensor, board: torch.Tensor = None) -> tuple:
        ...
        # Pass through expert layers with blending and layer norm
    ...
    def select_action(self, state: list, board: list, reward: float, done: bool) -> int:
        ...
        with self._lock:
            ...
            with torch.no_grad():
                ...
                # Store blend weights for real-time visualization
    ...
    def _train_step(self, device: str, grad_clip: float = 1.0):
        ...
        for epoch in range(self.ppo_epochs):
            ...
            for i in range(0, n_samples, self.batch_size):
                ...
                # Expert load balancing loss (very light - allow specialization)
                # Only penalize if one expert gets > 80% of usage (collapse prevention)
                avg_blend = blend.mean(dim=0)  # Average expert usage in batch
                max_usage = avg_blend.max()
                load_balance_loss = torch.relu(max_usage - 0.8) ** 2  # Only penalize extreme collapse
                ...
                with torch.no_grad():
                    ...
                    if approx_kl.item() > 0.03:  # Slightly more permissive
                        break
        ...
        # Emit average expert weights from the batch
    ...
    def last_blend_weights(self) -> list:
        """Get the most recent expert blend weights for visualization."""
