...
- Configurable rewards (legacy numeric or formula-based)
- Death reason tracking
- Multiple observation modes (vision settings)
"""
...
class SnakeEnv(BaseEnvironment):
    ...
    Supports both legacy numeric rewards and formula-based reward configurations.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            vision: Vision mode for observations
                    0 = immediate danger (4 directions)
                    >0 = window around head
                    -1 = full grid
            seg_size: Number of body segments to track in observation
            reward_config: Reward configuration (RewardConfig, dict, or None for defaults)
        """
        self.grid_size = grid_size
        self.vision = vision
        self.seg_size = seg_size
        self.starvation_limit = grid_size ** 2  # Max steps without food
    ...
    def step(self, action: Any) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if (head["x"] < 0 or head["x"] >= self.grid_size or
            ...
            self.last_reward = self._reward_config.get_legacy_reward(RewardEvent.DEATH_WALL)
            return self._get_observation(), self.last_reward, True, self._get_info()
        ...
        if any(seg["x"] == head["x"] and seg["y"] == head["y"]
            ...
            self.last_reward = self._reward_config.get_legacy_reward(RewardEvent.DEATH_SELF)
            return self._get_observation(), self.last_reward, True, self._get_info()
        ...
        if head["x"] == self.food_position["x"] and head["y"] == self.food_position["y"]:
            self.score += 1
            self.last_reward = self._reward_config.get_legacy_reward(RewardEvent.APPLE_EATEN)
            self.hunger = 0
            self.food_position = self._place_food()
            ...
            self.hunger += 1
            base_step_reward = self._reward_config.get_legacy_reward(RewardEvent.STEP)
            self.last_reward = base_step_reward * (self.hunger + 1)

            # Check starvation
            if self.hunger >= self.starvation_limit:
                self.game_over = True
                self.death_reason = "starvation"
                self.last_reward = self._reward_config.get_legacy_reward(RewardEvent.DEATH_STARVATION)
                return self._get_observation(), self.last_reward, True, self._get_info()

            # Remove tail (snake moves)
            self.snake_position.pop()

        return self._get_observation(), self.last_reward, False, self._get_info()

    def _apply_action(self, action: Any, head: Dict) -> None:
    ...
    def _place_food(self) -> Dict[str, int]:
    ...
    def render_state(self) -> Dict:
        """
        Render state for frontend visualization.

        Returns:
            Dictionary with game state suitable for JSON serialization
        """
        return {
            "score": self.score,
...
# Factory function for creating snake environments
def make_snake_env(
    grid_size: int = 10,
    vision: int = 0,
    seg_size: int = 3,
